...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train Atlas Stage 1B geometry + ego bootstrap.")
    ...
    cfg = load_trainer_config("stage1b", args.config)
    if args.variant:
    ...
    if args.init_checkpoint:
        cfg.stage1b.init_checkpoint = args.init_checkpoint
    cfg.train.use_privileged_dataset = True
    cfg.val.use_privileged_dataset = True

    result = run_training_stage("stage1b", cfg)
    print(f"stage1b step={result['step']} checkpoint_dir={result['checkpoint_dir']}")
