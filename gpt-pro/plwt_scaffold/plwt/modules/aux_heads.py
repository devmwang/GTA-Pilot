...
from ..state import WorldMemoryView
from .common import LearnedQueryPool, MLP
...
class LaneSegmentHead(nn.Module):
    ...
    def forward(self, world: WorldMemoryView) -> dict[str, torch.Tensor]:
...
class MapElementHead(nn.Module):
    ...
    def forward(self, world: WorldMemoryView, lane_queries: torch.Tensor) -> dict[str, torch.Tensor]:
...
class AuxHeads(nn.Module):
    ...
    def forward(self, world: WorldMemoryView, active_heads: Optional[Iterable[str]] = None) -> dict[str, torch.Tensor]:
