...
from .common import SelfAttentionBlock, sinusoidal_embedding, assert_rank
...
class TemporalMixer(nn.Module):
    ...
    def forward(self, cam_tokens: torch.Tensor, dt_hist: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if n != self.cfg.visual.cam_tokens_per_frame:
            raise ValueError(f"Expected {self.cfg.visual.cam_tokens_per_frame} camera tokens, got {n}")
