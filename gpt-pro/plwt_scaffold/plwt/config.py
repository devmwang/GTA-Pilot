...
class ImageConfig:
    ...
    @property
    def fy(self) -> float:
        vfov = 2.0 * math.atan(math.tan(math.radians(self.hfov_deg) / 2.0) * (self.raw_height / self.raw_width))
...
class HeadSchedulerConfig:
    drive_mode_cadence: Dict[str, int] = field(default_factory=lambda: {
        "planner": 1,
        "ego": 1,
        "lane": 1,
        "map": 1,
    ...
    inspect_mode_cadence: Dict[str, int] = field(default_factory=lambda: {
        "planner": 1,
        "ego": 1,
        "lane": 1,
        "map": 1,
...
class PLWTConfig:
    ...
    visual: VisualTokenizerConfig = field(default_factory=VisualTokenizerConfig)
    ...
    reasoner_adapter: ReasonerAdapterConfig = field(default_factory=ReasonerAdapterConfig)
...
def plwt_s1080_config() -> PLWTConfig:
    cfg = PLWTConfig(variant="PLWT-S1080", hidden_dim=448)
    cfg.visual.token_dim = 448
    cfg.visual.cam_tokens_per_frame = 192
    cfg.visual.ctx_dims = (128, 256, 384, 512)
    cfg.visual.ctx_blocks = (2, 4, 8, 4)
    cfg.visual.freeze_stages = 2
...
def plwt_t1080_priv_config() -> PLWTConfig:
    cfg = PLWTConfig(variant="PLWT-T1080-Priv", hidden_dim=640)
    cfg.visual.token_dim = 640
    cfg.visual.detail_dim = 96
    cfg.visual.ctx_dims = (160, 320, 512, 640)
    cfg.visual.ctx_blocks = (3, 6, 10, 6)
    cfg.visual.cam_tokens_per_frame = 256
    cfg.visual.query_pool_heads = 10
    cfg.visual.freeze_stages = 1
...
def plwt_smoke_config() -> PLWTConfig:
    cfg = PLWTConfig(variant="PLWT-SMOKE", hidden_dim=64)
    cfg.image.raw_height = 64
    cfg.image.raw_width = 96
    cfg.image.padded_height = 64
    cfg.image.padded_width = 96
    cfg.visual.token_dim = 64
    cfg.visual.detail_dim = 16
    cfg.visual.ctx_dims = (24, 32, 48, 64)
    cfg.visual.ctx_blocks = (1, 1, 2, 1)
    cfg.visual.cam_tokens_per_frame = 8
    cfg.visual.query_pool_heads = 2
